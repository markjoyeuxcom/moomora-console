...
test('CodeMirror editor exposes a high contrast caret and selection', () => {
  assert.match(styles, /\.code-editor\s+\.cm-cursor,\s*[\s\S]*\.code-editor\s+\.cm-dropCursor\s*\{[\s\S]*border-left-color:\s*var\(--caret\)\s*!important;/);
  assert.match(styles, /\.code-editor\s+\.cm-selectionBackground,\s*[\s\S]*\.code-editor\s+\.cm-content\s+::selection\s*\{[\s\S]*color-mix\(in srgb, var\(--accent\) 20%, transparent\)/);
});
