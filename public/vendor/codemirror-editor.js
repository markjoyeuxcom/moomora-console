  ...
  LinkEnd(cx, next, start) {
    ...
    for (let i = cx.parts.length - 1; i >= 0; i--) {
      ...
      if (part instanceof InlineDelimiter && (part.type == LinkStart || part.type == ImageStart)) {
        ...
        let link2 = cx.parts[i] = finishLink(cx, content2, part.type == LinkStart ? Type.Link : Type.Image, part.from, start + 1);
...
function findSectionEnd(headerNode, level) {
  let last = headerNode;
  for (; ; ) {
    let next = last.nextSibling, heading3;
    if (!next || (heading3 = isHeading(next.type)) != null && heading3 <= level)
      break;
    last = next;
  }
  return last.to;
...
var headerIndent = /* @__PURE__ */ foldService.of((state, start, end) => {
  for (let node = syntaxTree(state).resolveInner(end, -1); node; node = node.parent) {
    ...
    if (heading3 == null)
      continue;
    let upto = findSectionEnd(node, heading3);
    if (upto > end)
...
var pasteURLAsLink = /* @__PURE__ */ EditorView.domEventHandlers({
  paste: (event, view) => {
    ...
    if (main.empty)
      return false;
    let link2 = (_a2 = event.clipboardData) === null || _a2 === void 0 ? void 0 : _a2.getData("text/plain");
...
function prefixSelectedLines(value, selectionStart, selectionEnd, marker, fallback) {
  ...
  const { lineStart, lineEnd: lineEnd2 } = selectedLineRange(value, selectionStart, selectionEnd);
  const selectedLines = value.slice(lineStart, lineEnd2);
  const nextLines = selectedLines.split("\n").map((line) => `${marker}${line}`).join("\n");
  return {
    value: `${value.slice(0, lineStart)}${nextLines}${value.slice(lineEnd2)}`,
...
function heading2(value, selectionStart, selectionEnd) {
...
function applyMarkdownFormat(value, selectionStart, selectionEnd, action) {
  ...
  switch (action) {
    ...
    case "heading":
      return heading2(safeValue, start, end);
    case "bullet-list":
...
// scripts/codemirror-editor-source.js
var operatorTheme = EditorView.theme({
...
var operatorHighlight = HighlightStyle.define([
...
function mountCodeMirrorEditor({
  host,
  value = "",
  onChange,
  onSave
